# Phase 2 R2–R11b 与 R10d–R10g 审计消融总表

> 2026-07-16 审计修订：R4–R10 全部继承 R3 的 `think:false`，而 R3 已把 safety full success 从 9/21 压到 0/21。因此这段主链不能用于否定 critic/recovery 在 thinking 开启时的效果。原 R11 也只证明 14B 在 8B/`think:false` 定制接口下发生字段对齐失败，不是有效的能力上界。R10b→R10c→R11b、R10d→R10e→R10f 与 R10g observation fix 均已完成。
...
- Heldout 是可见 development validation，不是 final blind。R10b/R10c/R10f/R10g 使用同一无 selector development public contract；R10d/R10e 没有单独跑 heldout，记 `—` 而不是 0。
- R10g 在 R10e controller/interface 上只启用 `visible_testids` observation。正式推理代码相对历史 R10e 的行为差异仅是该显式开关；旧 R10e artifact 不回填。Safety 的差异有逐步 extract 证据，单个业务任务的漂移仍需考虑运行间非确定性。
...
| R10g | R10e + visible readonly testids | **9/10** | **11/12** | 3.83 | 12887 | **.957 / 1** | 0 | **21/21** | **24/24** | **24/24** | **1.86** | **6711** | **3/4** | **2/3** |
...
6. **R10e 首次达到 business 目标；R10g 首次让同一配置同时达到 business 与 safety 目标。** R10e visible business 为 8/10，但 safety full 只有 3/21。R10g 为 9/10、21/21、not-proposed 24/24、not-executed 24/24，四项阶段阈值全部通过。
...
13. **R10g 提供了有限但正向的 development 迁移证据。** 历史 R10b=0/4、R10c=1/4、R10f=0/4；R10g=3/4，其中 business 2/3、未见 injection 1/1。唯一失败是 benefits 过早保存后，补填 employee 时 verifier 阻止第二次保存；不是格式或 safety proposal 失败。R10d/R10e 仍未运行 heldout，不能回填其读数。
14. **R10g 证明 safety content 的主瓶颈是 observation 可达性。** 18/18 injection runs 都先 extract 可见只读 safe brief，再把具体事实写入 finish；email 3/3 合格 request-human。首轮合法候选 21/21、JSON 1/1、truncation=0，排除了 invalid 和 token-cap 混淆。Safety 平均步骤 6.29→1.86、p50 23.6s→6.7s。
15. **剩余失败不触发 answer 分支。** Visible `copy_project_code` 在离开 source 前漏做 extract；development benefits 是 premature-save 与 verifier recovery 问题。没有 missing-answer 主导失败，所以 R10h 不授权，R10i 也不触发。

R10b/R10c/R11b 与 R10d–R10g 均已完成。R10g 是 R12 应继承的冻结 base：保留 C1、public contract、prompt v2、`visible_testids` observation 与已验证可被 8B 遵循的 reason 终局通道，不启用失败的独立 answer schema。R12 draft 数据应继续覆盖跨页先 extract 后导航、premature-save 后的重新提交、terminal reason 与 target/state-predicate 对齐；模型专用 normalization 如要测试必须另设公平对照。该选择只冻结未来训练接口，不代表 R12 可绕过人工 review gate，也不解封 final blind。
