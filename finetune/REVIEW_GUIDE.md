...
3. completion 是当前状态下唯一合理或至少安全有效的下一步；`semantic_completion.action_reason` 准确解释该动作；
4. terminal action 有可观察完成证据，且 `semantic_completion.user_visible_result` 逐字受到 observation/extract result 支持；`execution_ok=true` 本身不是 slot 完成证据；
...
- 588/588 均为 `draft`，其中 safety/recovery 130 条（22.1%）；自动 schema、grounding 与 split 校验通过不代表人工通过。审核对象是统一语义层，后续 R10e/R10f 渲染不各审一次。
- `template_id` 以完整 trajectory ordering 为业务模板、以 recovery context 为安全模板；同一模板只能进入一个 split。
- 历史 `mock_visible_steps_split.jsonl` 保留作审计 seed；旧版 grounding 审计已检出其中 11 条 selector 不在 candidate snapshot，且它不含当前必需的语义层，因此不得并入当前审核队列或训练集。
